...
class Agent:
    ...
    def choose_action(self,observation,validation=False):
        if self.time_step < self.warmup and not validation:
            mu = torch.tensor(np.random.normal(scale=self.noise, size=(self.n_actions,)), 
                              dtype=torch.float32).to(self.actor.device)
    ...
    def learn(self):
        ...
        states, actions, rewards, next_state, dones = self.memory.sample_buffer(self.batch_size)

        states = torch.tensor(states,dtype=torch.float32).to(self.critic_1.device)
        actions = torch.tensor(actions,dtype=torch.float32).to(self.critic_1.device)
        rewards = torch.tensor(rewards,dtype=torch.float32).to(self.critic_1.device)
        next_state = torch.tensor(next_state,dtype=torch.float32).to(self.critic_1.device)
        dones = torch.tensor(dones, dtype=torch.bool).to(self.critic_1.device)


        target_actions = self.target_actor.forward(next_state)
        target_actions = target_actions + torch.clamp(
    torch.tensor(np.random.normal(scale=0.2), dtype=torch.float32), -0.5, 0.5
)
    ...
    def update_network_parameters(self,tau=None):
        ...
        for name in critic_1_state_dict:
            critic_1_state_dict[name] = (tau * critic_1_state_dict[name].to(torch.float32).clone()) + \
                                        ((1 - tau) * target_critic_1_state_dict[name].to(torch.float32).clone())
        for name in critic_2_state_dict:
            critic_2_state_dict[name] = (tau * critic_2_state_dict[name].to(torch.float32).clone()) + \
                                        ((1 - tau) * target_critic_2_state_dict[name].to(torch.float32).clone())


        for name in actor_state_dict:
            actor_state_dict[name] = (tau * actor_state_dict[name].to(torch.float32).clone()) + \
                                     ((1 - tau) * target_actor_state_dict[name].to(torch.float32).clone())
